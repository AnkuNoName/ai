...
def ankudevich_confusion_matrix(y_true, y_pred):
    TP, FN, FP, TN = find_conf_matrix_values(y_true, y_pred)
    return np.array([[TN, FP], [FN, TP]])


ankudevich_confusion_matrix(df.actual_label.values, df.predicted_RF.values)

assert np.array_equal(ankudevich_confusion_matrix(df.actual_label.values, df.predicted_RF.values),
                      confusion_matrix(df.actual_label.values,
                                       df.predicted_RF.values)), 'my_confusion_matrix() is not correct for RF'
assert np.array_equal(ankudevich_confusion_matrix(df.actual_label.values, df.predicted_LR.values),
                      confusion_matrix(df.actual_label.values,
...
def ankudevich_f1_score(y_true, y_pred):  # calculates the F1 score
    recall = ankudevich_recall_score(y_true, y_pred)
    precision = ankudevich_precision_score(y_true, y_pred)
    return 2 * (precision * recall) / (precision + recall)


assert ankudevich_f1_score(df.actual_label.values, df.predicted_RF.values) == f1_score(df.actual_label.values,
                                                                                  df.predicted_RF.values), 'my_accuracy_score failed on RF'
assert ankudevich_f1_score(df.actual_label.values, df.predicted_LR.values) == f1_score(df.actual_label.values,
                                                                                  df.predicted_LR.values), 'my_accuracy_score failed on LR'

print('F1 RF: %.3f' % (ankudevich_f1_score(df.actual_label.values, df.predicted_RF.values)))
print('F1 LR: %.3f' % (ankudevich_f1_score(df.actual_label.values, df.predicted_LR.values)))
...
print('Accuracy RF: % .3f' % (ankudevich_accuracy_score(df.actual_label.values, (df.model_RF >= threshold).astype('int').values)))
print('Recall RF: %.3f' % (ankudevich_recall_score(df.actual_label.values, (df.model_RF >= threshold).astype('int').values)))
print('Precision RF: %.3f' % (ankudevich_precision_score(df.actual_label.values, (df.model_RF >= threshold).astype('int').values)))
print('F1 RF: %.3f' % (ankudevich_f1_score(df.actual_label.values, (df.model_RF >= threshold).astype('int').values)))
